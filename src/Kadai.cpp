...
void jankenController()
{
	...
	Sleep(1000);
	if (winCount < 3) {
		printf("あなたは『じゃんけん見習い』です。\n");
		printf("まだまだこれからですね。もっとこのゲームで遊んで腕を磨きましょう。\n");
	}
	else if (3 <= winCount && winCount < 7) {
		printf("あなたは『じゃんけんリーダー』です。\n");
		printf("腕前が上がってきましたね。ですが、まだまだ先は長いですよ。\n");
	}
	else if (7 <= winCount && winCount < 10) {
		printf("あなたは『じゃんけんプロフェッショナル』です。\n");
		printf("ようやくここまで来ましたね。頂点まであと少しです。\n");
	}
	else if (10 <= winCount) {
		printf("あなたは『じゃんけんマスター』です。\n");
		printf("ここまで来たあなたは、じゃんけんにおいては無敵です。\n");
	}
	//ゲーム終了時のインターバル
	Sleep(500);
}
